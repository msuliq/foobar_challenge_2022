...
integers denoting where the groups of gathered bunnies are, an array of integers
denoting where the escape pods are located, and an array of an array of integers
of the corridors, returning the total number of bunnies that can get through at
each time step as an int. The entrances and exits are disjoint and thus will
never overlap. The path element path[A][B] = C describes that the corridor going
...
-- Java cases --
Input:
Solution.solution({0, 1}, {4, 5}, {{0, 0, 4, 6, 0, 0},
                                   {0, 0, 5, 2, 0, 0},
                                   {0, 0, 0, 0, 4, 4},
                                   {0, 0, 0, 0, 6, 6},
                                   {0, 0, 0, 0, 0, 0},
...
def solution(entrances, exits, path):
    ...
    for i in range(len(path)):
        ...
        for j in range(len(path[i])):
            augr[i].append([0, path[i][j]])
        augr[i].append([0, 0])
        if i in exits:
            augr[i].append([0, max_val])
        else:
            augr[i].append([0, 0])
    augr.append([])
    augr.append([])
    for i in range(len(path[0]) + 2):
        if i in entrances:
            augr[-2].append([0, max_val])
        else:
            augr[-2].append([0, 0])
        augr[-1].append([0, 0])
    while bfs(augr, len(augr)-2, len(augr)-1):
        pass
    total = 0
    for i in range(len(augr)):
        total += augr[-2][i][0]
    return total
