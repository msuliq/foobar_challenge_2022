...
Oh no! Commander Lambda's latest experiment to improve the efficiency of the
LAMBCHOP doomsday device has backfired spectacularly. The Commander had been
...
Commander Lambda is having her henchmen rebuild the ion flux converter tree by
hand, but you think you can do it much more quickly -- quickly enough, perhaps,
to earn a promotion!

Flux chains require perfect binary trees, so Lambda's design arranged the ion
flux converters to form one. To label them, Lambda performed a post-order
traversal of the tree of converters and labeled each converter with the order of
that converter in the traversal, starting at 1. For example, a tree of 7
...
def solution(h, q=[]):
    ...
    for x in q:
        if(True):
            ...
            if(x==number):
                # if number is binary tree root append -1
                answer.append(-1)
            else:
                # if not root seach down
                node_number=int(math.pow(2,current_height)-2)
                # determine number to the left and right of a node
                left_leaf_number=int(number-int(node_number/2)-1)
                right_leaf_number=int(number-1)
                # mark current node as scanned
                flag=0
                # while x is not in left or right leaf move to next node
                while(x!=right_leaf_number and x!=left_leaf_number and flag<10):
                    flag=flag+1
                    # x is right leaf
                    if(x>left_leaf_number):
                        number=right_leaf_number
                    # x is left leaf
                    else:
                        number=left_leaf_number
                    # set vars to search further down
                    current_height=current_height-1
                    node_number=int(math.pow(2,current_height)-2)
                    left_leaf_number=number-(node_number/2)-1
                    right_leaf_number=number-1
