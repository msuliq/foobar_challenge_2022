...
allow for hours-long lines, so the trainers at the checkpoint have found ways to
quicken the pass-through rate. Instead of checking each and every worker coming
through, the bunny trainers instead go over everyone in line while noting their
worker IDs, then allow the line to fill back up. Once they've done that they go
over the line again, this time leaving off the last worker. They continue doing
this, leaving off one more worker from the line each time but recording the
worker IDs of those they do check, until they skip the entire line, at which
point they XOR the IDs of all the workers they noted into a checksum and then
take off for lunch. Fortunately, the workers' orderly nature causes them to
...
def solution2(start, length):
    ...
    i = 0
    result = 0
    while number < (number+(length*length)) and temp_length > 0:
        #if required condition is not satisfied (when it reaches '/' in every row)  
        if i == temp_length:   
            i = 0
            number += length - temp_length
            temp_length -= 1
...
print(solution(0, 3))
print(solution(17, 4))
print(solution(2000000000, 50000))

print(solution2(0, 3))
print(solution2(17, 4))
print(solution2(2000000000, 50000))